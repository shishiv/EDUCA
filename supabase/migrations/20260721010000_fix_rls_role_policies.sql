...
-- All helpers gate on ativo = true so inactive users resolve to no role and
-- no school, matching the pilot contract (inactive users have no access).
...
AS $$
  SELECT tipo_usuario FROM public.users WHERE id = auth.uid() AND ativo = true LIMIT 1;
$$;
...
SET search_path = public
AS $$
  SELECT escola_id FROM public.users WHERE id = auth.uid() AND ativo = true LIMIT 1;
$$;
...
CREATE POLICY alunos_select_authorized ON public.alunos
  FOR SELECT TO authenticated
  USING (
    public.has_any_role('admin', 'gestor_sme')
    OR (public.has_any_role('secretario') AND public.current_user_school_id() IS NULL)
    OR (
      public.has_any_role('diretor', 'secretario', 'professor')
      AND EXISTS (
        SELECT 1 FROM public.matriculas m
        JOIN public.turmas t ON t.id = m.turma_id
        WHERE m.aluno_id = alunos.id
          AND public.can_access_school(t.escola_id)
      )
    )
  );
  ...
  USING (
    ...
    OR (
      ...
      AND EXISTS (
        ...
        JOIN public.matriculas m ON m.aluno_id = ar.aluno_id
        JOIN public.turmas t ON t.id = m.turma_id
  ...
  USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM public.matriculas m
        JOIN public.turmas t ON t.id = m.turma_id
        WHERE m.aluno_id = aluno_responsaveis.aluno_id
          AND public.can_access_school(t.escola_id)
      )
    )
  );
