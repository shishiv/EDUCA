-- Auth mutation audit receipts coordinated for quality campaign Q-C10-01.
-- Extends the existing controlled RPC; historical migrations remain immutable.
...
BEGIN
  ...
  IF p_event_type NOT IN (
    ...
    'profile_updated', 'user_updated', 'user_status_updated'
  ) THEN
  ...
  ELSIF p_event_type IN ('first_access_completed', 'profile_updated') THEN
    IF p_entity_type <> 'user' OR p_entity_id IS DISTINCT FROM auth.uid()::text THEN
      RAISE EXCEPTION 'PILOT_AUDIT_ENTITY_DENIED: self-service change must audit the current user';
    END IF;
    ...
    END IF;
    p_escola_id := target_school;
  ELSIF p_event_type IN ('user_updated', 'user_status_updated') THEN
    IF actor_role <> 'admin' OR p_entity_type <> 'user'
       OR p_entity_id IS NULL
       OR p_entity_id !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
      RAISE EXCEPTION 'PILOT_AUDIT_ROLE_DENIED: managed user changes require an administrator and user target';
    END IF;
    SELECT escola_id INTO target_school FROM public.users WHERE id = p_entity_id::uuid;
    IF NOT FOUND OR p_escola_id IS DISTINCT FROM target_school THEN
      RAISE EXCEPTION 'PILOT_AUDIT_ENTITY_DENIED: managed user target school is not authoritative';
    END IF;
  ...
  ELSIF p_event_type IN ('import_staged', 'import_published') THEN
    ...
    END IF;
    SELECT escola_id, submitted_by, approved_by INTO batch_row
    FROM public.pilot_import_batches WHERE id = p_entity_id::uuid;
    IF NOT FOUND THEN
  ...
  ELSIF p_event_type = 'whatsapp_optin_changed' THEN
    ...
    END IF;
    SELECT escola_id INTO target_school FROM public.responsaveis WHERE id = p_entity_id::uuid;
    ...
    AND NOT EXISTS (
      ...
      WHERE metadata_key.key <> ALL (
        CASE $1
          ...
          WHEN 'first_access_completed', 'profile_updated', 'user_updated', 'user_status_updated' THEN ARRAY[]::text[]
          ELSE ARRAY[]::text[]
        END
      )
    );
