...
mapfile -t tables < <(awk -F '\t' '$1 == "included" { print $2 }' "$PROOF_DIR/restore-coverage-v1.tsv")
[[ ${#tables[@]} == 18 ]] || { echo 'RESTORE_ALLOWLIST_CHANGED: F07 review required' >&2; exit 1; }
"${PSQL[@]}" -d restored -f "$PROOF_DIR/restore-catalog.sql" > "$WORK_DIR/expected.catalog"
"${PSQL[@]}" -d restored -c 'TRUNCATE public.pilot_municipality_config CASCADE'
for table in "${tables[@]}"; do
  "${PSQL[@]}" -d source -c "\copy (SELECT * FROM public.\"$table\" ORDER BY id) TO '$WORK_DIR/$table.csv' WITH (FORMAT csv, HEADER true)"
  "${PSQL[@]}" -d restored -c 'SET session_replication_role = replica; SET ROLE service_role' \
...
echo 'RESTORE_RAW_PARTIAL_OK: 18 table CSV comparisons (empty tables identified), identity manifest and migration catalog; no Storage bytes, encryption or GoTrue session proof'
