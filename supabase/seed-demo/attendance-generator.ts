 ...
 *    days = 1000 frequencia rows and 5 turmas x 20 days = 100 aulas.
    ...
    days.forEach((dataAula, dayIndex) => {
      ...
      for (let offset = 0; offset < 10; offset += 1) {
        ...
        frequencia.push({
          ...
          dataAula,
          presente,
          statusPresenca: presente ? 'presente' : 'ausente',
...
export function attendanceSql(options: AttendanceGenerationOptions): string {
  ...
  const aulaRows: Array<Array<string | number | boolean | null>> = aulas.map(aula => [
    aula.id,
    aula.turmaId,
    aula.escolaId,
    aula.professorId,
    ...
    aula.disciplina,
    aula.status,
    aula.tempoLimiteMinutos,
    aula.abertaEm,
    aula.fechadaEm,
    aula.createdAt,
    // updated_at has a now() default - set it deterministically so resets
    // with the same anchor produce byte-identical rows.
    aula.createdAt,
  ])
  ...
  return [
    ...
    emitMultiRowInsert(
      'aulas_abertas',
      ['id', 'turma_id', 'escola_id', 'professor_id', 'data_aula', 'disciplina', 'status', 'tempo_limite_minutos', 'aberta_em', 'fechada_em', 'created_at', 'updated_at'],
      aulaRows
    ),
    emitMultiRowInsert(
      'frequencia',
      ['id', 'matricula_id', 'data_aula', 'presente', 'status_presenca', 'justificativa', 'professor_id', 'marcado_por', 'marcado_em', 'aula_id', 'created_at'],
      frequenciaRows
    ),
  ].join('\n')
}
