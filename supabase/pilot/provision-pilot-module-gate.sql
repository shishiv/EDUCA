...
-- C06 owns school mutation through governed security-definer RPCs. Revoke the
-- legacy pilot grant, including its column grants, before restoring the
-- read-only browser contract.
REVOKE ALL ON TABLE public.escolas FROM authenticated;
GRANT SELECT ON TABLE public.escolas TO authenticated;
