...
- Bolsa Família and NIS reads are denied on `alunos` for browser roles and exposed only through policy-checked RPCs. Each school owns a `configs.bolsa_familia_visible_roles` allowlist with the default `admin,diretor,secretario`; teachers and guardians cannot be added. Coordination and social-assistance duties use the existing administrative roles rather than new role values.
