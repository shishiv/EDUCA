...
- `app/app/globals.css` holds the public marketing and app-shell rules inside `@layer components`. Tailwind 3 only emits a rule there when its exact class string appears in a file scanned by `app/tailwind.config.js`, so those class names must be written as literals in JSX and never composed at runtime.
...
- The public landing preview (`ProductBoard` in `app/components/marketing/educa-landing.tsx`) states that it reproduces the authenticated Overview screen, so it reads its labels from that screen's catalogs (`platform.dashboard`, `layout.navigation.items`) and keeps every value a dash. Change the metrics or the class list in `app/app/(dashboard)/dashboard/page.tsx` and either follow it here or correct the caption; `app/tests/unit/i18n/redesign-contract.test.tsx` guards the pairing.
...
- General attendance bands are persisted in `configs.attendance_alert_bands`, seeded at 80/85 with whole-pair school overrides. Only municipal `admin`/`secretario` actors without a school may edit them through the existing municipal settings RPC. `app/lib/attendance/resolve-attendance-bands.ts` resolves and validates the school policy for dashboard and exports; missing configuration is an error. These bands do not determine approval or Bolsa Família eligibility. See [`docs/GOVERNED-ATTENDANCE-BANDS.md`](docs/GOVERNED-ATTENDANCE-BANDS.md) for authority, consumer classification, deliberately preserved legacy legal/approval rules and executable proof.
