...
export type MatriculasDependencies = { supabaseClient: Pick<typeof supabase, 'from' | 'rpc'> }
...
export function MatriculasPageContent({ supabaseClient }: MatriculasDependencies) {
  ...
  const handleDeleteMatricula = async (matriculaId: string, alunoNome: string) => {
    ...
    try {
      const matricula = matriculas.find(candidate => candidate.id === matriculaId)
      await updateGovernedEnrollment(
        supabaseClient,
        matriculaId,
        'cancelada',
      ...
      logger.error('Error cancelling matricula', normalizedError, {
        metadata: {
      ...
      toast.error(`Erro ao cancelar matrícula: ${normalizedError.message || 'Tente novamente.'}`)
    }
  }
