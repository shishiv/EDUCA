...
function isAttendanceLockError(error: z.infer<typeof AttendanceApiFailureSchema>): boolean {
  ...
  return message.includes('18:00') || message.includes('bloqueado') || message.includes('locked')
}
...
export function AttendanceGrid({
  ...
  const [lockInfo, setLockInfo] = useState<SessionLockInfo>(() =>
    getSessionLockInfo(sessionDate, sessionStatus, correctionDeadlineAt)
  )
  ...
  useEffect(() => {
    const updateLockInfo = () => {
      setLockInfo(getSessionLockInfo(sessionDate, sessionStatus, correctionDeadlineAt))
    }
    updateLockInfo()
