...
export default defineConfig({
  ...
  projects: [
    ...
    // ─── Admin (default authenticated role) ─────────────────────────────────
    {
      ...
      // and public-origin runners as well. Positive grades contracts are held
      // separately while canonical notas RLS has no browser policies.
      testIgnore: [
        /.*\/(auth|pilot|pilot-descriptive|public-demo)\/.*/,
        '**/grades/entry.spec.ts',
        '**/grades/report-card.spec.ts',
      ],
      dependencies: ['setup'],
    },
