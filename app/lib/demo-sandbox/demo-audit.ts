...
import type {
  ...
  PilotRpcResult,
  PilotRpcReturn,
} from '@/lib/pilot/pilot-rpc-client'
...
export type WritePilotAuditEventArgs = PilotRpcArguments<'write_pilot_audit_event'> & {
...
export interface DemoAuditRpcClient {
  ...
  ): Promise<PilotRpcResult<PilotRpcReturn<'write_pilot_audit_event'>>>
}

export interface DemoActionAuditMetadata extends Record<string, Json> {
  operation: DemoActionOperation
  outcome: 'simulated_success'
