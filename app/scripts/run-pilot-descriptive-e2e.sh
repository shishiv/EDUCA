...
ISOLATED_PROJECT_DIR="$ROOT_DIR/.pilot-descriptive-supabase"
SUPABASE_CONFIG_DIR="$ISOLATED_PROJECT_DIR/supabase"
AUTH_STATE_PATH="$APP_DIR/playwright/.pilot-descriptive/teacher.json"
APP_PID=''

choose_port_base() {
  local candidate=${PILOT_DESCRIPTIVE_PORT_BASE:-55321}
...
export NO_COLOR=1
pnpm exec supabase --workdir "$ISOLATED_PROJECT_DIR" start >/dev/null
...
unset NEXT_PUBLIC_DEMO_SANDBOX DEMO_SANDBOX
...
psql "$DB_URL" -X -v ON_ERROR_STOP=1 -f "$ROOT_DIR/supabase/pilot/provision-pilot-descriptive-report-demo.sql" >/dev/null
pnpm exec tsx scripts/seed-pilot-descriptive.ts
pnpm exec tsx scripts/validate-pilot-descriptive.ts
printf 'PILOT_DESCRIPTIVE_PHASE: build\n'
pnpm build
...
for _ in $(seq 1 60); do
  BASE_URL=$(portless get educa-pilot-descriptive 2>/dev/null || true)
...
printf 'PILOT_DESCRIPTIVE_E2E_RECEIPT: named_url_ready database=%s\n' "$ISOLATED_PROJECT_DIR"
pnpm exec playwright test --config playwright.pilot-descriptive.config.ts
