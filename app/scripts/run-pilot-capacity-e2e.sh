...
RECEIPT_STEM="$RECEIPT_DIR/r3-t4-capacity-pilot-e2e-$RUN_ID"
...
CONCURRENCY_RECEIPT="$RECEIPT_STEM.concurrency.json"
...
choose_port_base() {
  ...
  if [[ ! "$candidate" =~ ^[0-9]+$ ]]; then
    echo 'PILOT_CAPACITY_E2E_PORT_BASE_INVALID: a leased PILOT_E2E_PORT_BASE is required' >&2
    return 1
  fi
  printf '%s' "$candidate"
}
...
write_setup_receipt() {
  local status="$1"
  PILOT_CAPACITY_SETUP_STATUS="$status" node <<'NODE'
const fs = require('node:fs')
const path = require('node:path')

const outputPath = process.env.PILOT_CAPACITY_SETUP_RECEIPT_PATH
const apiUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || ''
const databaseUrl = process.env.SUPABASE_DB_URL || ''
const appUrl = process.env.PILOT_CAPACITY_NAMED_APP_URL || process.env.NEXT_PUBLIC_APP_URL || ''
...
const receipt = {
  result: process.env.PILOT_CAPACITY_SETUP_STATUS || 'unknown',
  ...
  roleSetup: {
    role: 'diretor',
    identity: process.env.PILOT_CAPACITY_DIRECTOR_EMAIL,
    authStatePath: process.env.PILOT_CAPACITY_AUTH_STATE_PATH || null,
  },
  seed: {
    command: 'pnpm exec tsx scripts/seed-pilot-capacity.ts',
    validationCommand: 'pnpm exec tsx scripts/validate-pilot-capacity.ts',
  },
  flags: {
...
const fs = require('node:fs')
const path = require('node:path')
const outputPath = process.env.PILOT_CAPACITY_DATABASE_RECEIPT_PATH
const receipt = {
  result: 'pass',
  boundary: 'real-postgresql',
  seedMarker: 'SYNTHETIC-EDUCA-PILOT-CAPACITY',
  seedCommand: 'pnpm exec tsx scripts/seed-pilot-capacity.ts',
  validationCommand: 'pnpm exec tsx scripts/validate-pilot-capacity.ts',
  localDatabase: true,
  syntheticOnly: true,
  concurrencyContract: 'real PostgREST insert boundary is exercised by Playwright',
}
fs.mkdirSync(path.dirname(outputPath), { recursive: true })
fs.writeFileSync(outputPath, `${JSON.stringify(receipt, null, 2)}\n`, 'utf8')
NODE
...
write_test_receipt() {
  local status="$1"
  PILOT_CAPACITY_TEST_STATUS="$status" PILOT_CAPACITY_TEST_EXIT="$TEST_EXIT" node <<'NODE'
const fs = require('node:fs')
const path = require('node:path')

const outputPath = process.env.PILOT_CAPACITY_TEST_RECEIPT_PATH
const outputFile = process.env.PILOT_CAPACITY_PLAYWRIGHT_OUTPUT_PATH
const lineCount = fs.existsSync(outputFile)
  ? fs.readFileSync(outputFile, 'utf8').split(/\r?\n/).filter(Boolean).length
  : 0
const receipt = {
  result: process.env.PILOT_CAPACITY_TEST_STATUS,
  exitCode: Number(process.env.PILOT_CAPACITY_TEST_EXIT),
  boundary: 'real-playwright-browser-and-postgrest',
  config: 'playwright.pilot-capacity.config.ts',
  expectedTestCount: Number(process.env.PILOT_CAPACITY_EXPECTED_RUN_TEST_COUNT),
...
write_deliberate_break_receipt() {
  local target="$1"
  PILOT_CAPACITY_BREAK_TARGET="$target" node <<'NODE'
const fs = require('node:fs')
const path = require('node:path')
const outputPath = process.env.PILOT_CAPACITY_LIFECYCLE_BREAK_RECEIPT_PATH
const receipt = {
  result: 'expected-red',
  target: process.env.PILOT_CAPACITY_BREAK_TARGET,
...
cleanup() {
  ...
  redact_file "$CONCURRENCY_RECEIPT_TMP" "$CONCURRENCY_RECEIPT"
...
JSON
  ...
  PILOT_CAPACITY_SKIPPED="$SKIPPED" node - "$FINAL_RECEIPT" "$SETUP_RECEIPT" "$DATABASE_RECEIPT" "$ROLE_SETUP_RECEIPT" "$TEST_RECEIPT" "$CONCURRENCY_RECEIPT" "$DELIBERATE_BREAK_RECEIPT" "$CLEANUP_RECEIPT" "$TEST_OUTPUT_RECEIPT" "$exit_code" "$RESULT" "$BASE_URL" "$DELIBERATE_BREAK" <<'NODE'
const fs = require('node:fs')
const path = require('node:path')

const [finalPath, setupPath, databasePath, rolePath, testPath, concurrencyPath, breakPath, cleanupPath, outputPath, exitCode, result, baseUrl, deliberateBreak] = process.argv.slice(2)
const readJson = file => fs.existsSync(file) ? JSON.parse(fs.readFileSync(file, 'utf8')) : null
const relative = file => path.relative(process.cwd(), file)
const skipped = process.env.PILOT_CAPACITY_SKIPPED === 'true'
const receipt = {
  contract: 'R3-T4 dedicated capacity pilot E2E',
  result: skipped ? 'skipped' : Number(exitCode) === 0 ? 'passed' : Number(exitCode) === 130 || Number(exitCode) === 143 ? 'interrupted' : 'failed',
  exitCode: Number(exitCode),
  command: 'cd app && pnpm test:e2e:pilot:capacity',
  selectedManifest: {
    config: 'playwright.pilot-capacity.config.ts',
    setup: 'tests/e2e/pilot/capacity-auth.setup.ts',
    specs: ['tests/e2e/pilot/capacity-contract.spec.ts'],
    expectedTests: 2,
  ...
  seed: {
    ...
    marker: 'SYNTHETIC-EDUCA-PILOT-CAPACITY',
    command: 'pnpm exec tsx scripts/seed-pilot-capacity.ts',
    validationCommand: 'pnpm exec tsx scripts/validate-pilot-capacity.ts',
  },
  ...
  concurrency: readJson(concurrencyPath),
  lifecycleDeliberateBreak: readJson(breakPath),
  portRangeLease: {
    serverMode: process.env.PILOT_E2E_APP_SERVER || 'portless',
    external: process.env.PILOT_E2E_PORT_LEASE_EXTERNAL === 'true',
    leaseDir: null,
...
NODE
  ...
  printf 'PILOT_CAPACITY_CLEANUP_RECEIPT: app_stopped=%s database_stopped=%s auth_state_removed=%s isolated_removed=%s result=%s receipt=%s\n' \
...
if [[ "$APP_SERVER_MODE" == portless ]] && ! command -v portless >/dev/null; then
  echo 'PILOT_CAPACITY_E2E_PREREQUISITE_MISSING: portless' >&2
  exit 1
fi
...
case "$DELIBERATE_BREAK" in
  ...
  *)
    echo 'PILOT_CAPACITY_DELIBERATE_BREAK_INVALID: expected none, selection, safety, or cleanup' >&2
    exit 1
    ;;
esac
...
ISOLATED_PROJECT_DIR=$(mktemp -d "$ROOT_DIR/.pilot-r3-t4-capacity-supabase.XXXXXX")
SUPABASE_PROJECT_ID=$(basename "$ISOLATED_PROJECT_DIR")
SUPABASE_PROJECT_ID="${SUPABASE_PROJECT_ID#.}"
SUPABASE_CONFIG_DIR="$ISOLATED_PROJECT_DIR/supabase"
AUTH_DIR="$APP_DIR/playwright/.pilot-capacity/$RUN_ID"
AUTH_STATE_PATH="$AUTH_DIR/director.json"
...
CONCURRENCY_RECEIPT_TMP="$ISOLATED_PROJECT_DIR/concurrency-receipt.json"
DELIBERATE_BREAK_RECEIPT_TMP="$ISOLATED_PROJECT_DIR/deliberate-break-receipt.json"
...
if ! pnpm exec supabase --workdir "$ISOLATED_PROJECT_DIR" status -o env >"$STATUS_ENV_PATH" 2>"$STATUS_LOG"; then
  echo 'PILOT_CAPACITY_E2E_FAILED: phase=supabase_status' >&2
  show_log_on_failure "$STATUS_LOG"
  exit 1
fi
if ! grep -q '^PUBLISHABLE_KEY=' "$STATUS_ENV_PATH" || ! grep -q '^SECRET_KEY=' "$STATUS_ENV_PATH"; then
  echo 'PILOT_CAPACITY_E2E_FAILED: phase=supabase_status current key aliases are missing' >&2
  exit 1
fi
...
export PILOT_CAPACITY_EXPECTED_RUN_TEST_COUNT="$EXPECTED_RUN_TEST_COUNT"
export PILOT_CAPACITY_DIRECTOR_EMAIL='diretor.capacidade@synthetic.invalid'
export PILOT_CAPACITY_SETUP_RECEIPT_PATH="$SETUP_RECEIPT_TMP"
export PILOT_CAPACITY_DATABASE_RECEIPT_PATH="$DATABASE_RECEIPT_TMP"
export PILOT_CAPACITY_ROLE_SETUP_RECEIPT_PATH="$ROLE_SETUP_RECEIPT_TMP"
export PILOT_CAPACITY_TEST_RECEIPT_PATH="$TEST_RECEIPT_TMP"
export PILOT_CAPACITY_CONCURRENCY_RECEIPT_PATH="$CONCURRENCY_RECEIPT_TMP"
export PILOT_CAPACITY_LIFECYCLE_BREAK_RECEIPT_PATH="$DELIBERATE_BREAK_RECEIPT_TMP"
export PILOT_CAPACITY_PLAYWRIGHT_OUTPUT_PATH="$PLAYWRIGHT_OUTPUT_TMP"
export PILOT_CAPACITY_NAMED_APP_URL="$NEXT_PUBLIC_APP_URL"
...
if [[ "$DELIBERATE_BREAK" == safety ]]; then
  ...
  printf 'PILOT_CAPACITY_DELIBERATE_BREAK: target=safety expected=red\n'
fi
...
const localHosts = new Set(['127.0.0.1', 'localhost'])
if (!apiUrl || !localHosts.has(new URL(apiUrl).hostname)) throw new Error('PILOT_CAPACITY_PRE_RESET_LOCAL_API_REQUIRED')
if (!databaseUrl || !localHosts.has(new URL(databaseUrl).hostname)) throw new Error('PILOT_CAPACITY_PRE_RESET_LOCAL_DATABASE_REQUIRED')
if (process.env.NEXT_PUBLIC_PILOT_MODE !== 'true' || process.env.PILOT_MODE !== 'true' || process.env.PILOT_SYNTHETIC_DATA_ONLY !== 'true') throw new Error('PILOT_CAPACITY_PRE_RESET_SYNTHETIC_FLAGS_REQUIRED')
if (process.env.PILOT_EXTERNAL_DEPLOY_APPROVED !== 'false') throw new Error('PILOT_CAPACITY_PRE_RESET_EXTERNAL_DEPLOY_MUST_BE_FALSE')
if (process.env.PILOT_LEGAL_APPROVAL_STATUS !== 'not_approved') throw new Error('PILOT_CAPACITY_PRE_RESET_LEGAL_APPROVAL_MUST_BE_NOT_APPROVED')
if (!process.env.PUBLISHABLE_KEY?.startsWith('sb_publishable_')) throw new Error('PILOT_CAPACITY_PRE_RESET_PUBLISHABLE_ALIAS_REQUIRED')
if (!process.env.SECRET_KEY?.startsWith('sb_secret_')) throw new Error('PILOT_CAPACITY_PRE_RESET_SECRET_ALIAS_REQUIRED')
const namedUrl = new URL(appUrl)
if (process.env.PILOT_E2E_APP_SERVER === 'direct') {
  if (!['127.0.0.1', 'localhost'].includes(namedUrl.hostname) || namedUrl.protocol !== 'http:') throw new Error('PILOT_CAPACITY_PRE_RESET_DIRECT_ORIGIN_REQUIRED')
} else if (!namedUrl.hostname.endsWith('.localhost') || namedUrl.port !== '') throw new Error('PILOT_CAPACITY_PRE_RESET_NAMED_APP_URL_REQUIRED')
console.log('PILOT_CAPACITY_PRE_RESET_SAFETY_RECEIPT: local=true synthetic_only=true external_deploy=false legal=not_approved current_aliases=true')
NODE
...
run_captured 'synthetic_seed' "$SEED_LOG" pnpm exec tsx scripts/seed-pilot-capacity.ts
run_captured 'database_validation' "$DATABASE_VALIDATION_LOG" pnpm exec tsx scripts/validate-pilot-capacity.ts
write_database_receipt
printf 'PILOT_CAPACITY_SEED_RECEIPT: seed=pass validation=pass marker=SYNTHETIC-EDUCA-PILOT-CAPACITY\n'
...
export PILOT_CAPACITY_NAMED_APP_URL="$BASE_URL"
run_captured 'ready_setup_receipt' "$ISOLATED_PROJECT_DIR/ready-setup-receipt.log" write_setup_receipt ready
node <<'NODE'
const value = process.env.PILOT_CAPACITY_NAMED_APP_URL
if (!value) throw new Error('PILOT_CAPACITY_NAMED_APP_URL_MISSING')
const url = new URL(value)
if (process.env.PILOT_E2E_APP_SERVER === 'direct') {
  if (!['127.0.0.1', 'localhost'].includes(url.hostname) || url.protocol !== 'http:') throw new Error('PILOT_CAPACITY_DIRECT_ORIGIN_REQUIRED')
} else if (!['http:', 'https:'].includes(url.protocol) || !url.hostname.endsWith('.localhost') || url.port !== '') {
  throw new Error('PILOT_CAPACITY_NAMED_APP_URL_INVALID')
}
console.log('PILOT_CAPACITY_BROWSER_SETUP_RECEIPT: named_url=true numbered_port=false')
NODE

run_captured 'manifest_list' "$MANIFEST_LIST_LOG" pnpm exec playwright test --config playwright.pilot-capacity.config.ts --list --reporter=line
selected_test_count=$(grep -Ec '^[[:space:]]+\[chromium-capacity\] ' "$MANIFEST_LIST_LOG" || true)
...
if [[ "$selected_test_count" -ne "$EXPECTED_TEST_COUNT" || "$setup_test_count" -ne "$EXPECTED_SETUP_TEST_COUNT" ]]; then
  echo "PILOT_CAPACITY_MANIFEST_RED: expected_tests=$EXPECTED_TEST_COUNT observed_tests=$selected_test_count observed_setup=$setup_test_count" >&2
  exit 1
fi
printf 'PILOT_CAPACITY_MANIFEST_RECEIPT: setup=1 specs=1 tests=%s descriptive=false r1_canonical=false\n' "$selected_test_count"

if [[ "$DELIBERATE_BREAK" == selection ]]; then
  write_deliberate_break_receipt selection
  printf 'PILOT_CAPACITY_DELIBERATE_BREAK: target=selection expected=red\n'
  echo 'PILOT_CAPACITY_MANIFEST_RED: deliberate selection break' >&2
  exit 1
fi

set +e
pnpm exec playwright test --config playwright.pilot-capacity.config.ts --reporter=line >"$PLAYWRIGHT_OUTPUT_TMP" 2>&1
TEST_EXIT=$?
set -e
if grep -Eiq '(^|[[:space:]])[0-9]+ skipped([[:space:]]|$)' "$PLAYWRIGHT_OUTPUT_TMP"; then
  SKIPPED=true
  write_test_receipt skipped
  echo 'PILOT_CAPACITY_E2E_SKIPPED: a selected pilot test was skipped' >&2
  exit 1
fi
if [[ "$TEST_EXIT" -ne 0 ]]; then
  write_test_receipt fail
  echo "PILOT_CAPACITY_E2E_FAILED: phase=browser status=$TEST_EXIT" >&2
...
if ! grep -Eq "[[:space:]]${EXPECTED_RUN_TEST_COUNT} passed([[:space:]]|$)" "$PLAYWRIGHT_OUTPUT_TMP"; then
  write_test_receipt fail
  echo "PILOT_CAPACITY_E2E_RESULT_COUNT_RED: expected=${EXPECTED_RUN_TEST_COUNT} passed" >&2
  show_log_on_failure "$PLAYWRIGHT_OUTPUT_TMP"
  exit 1
fi
write_test_receipt pass
if [[ ! -s "$CONCURRENCY_RECEIPT_TMP" ]]; then
  echo 'PILOT_CAPACITY_CONCURRENCY_RECEIPT_MISSING: real PostgREST concurrency receipt was not written' >&2
  exit 1
fi

if [[ "$DELIBERATE_BREAK" == cleanup ]]; then
  write_deliberate_break_receipt cleanup
  printf 'PILOT_CAPACITY_DELIBERATE_BREAK: target=cleanup expected=red\n'
  echo 'PILOT_CAPACITY_CLEANUP_RED: deliberate interruption before successful child completion' >&2
  exit 1
fi

RESULT='passed'
TEST_EXIT=0
printf 'PILOT_CAPACITY_TEST_RECEIPT: status=pass tests=%s concurrency_receipt=true output_redacted=true\n' "$EXPECTED_TEST_COUNT"
exit 0
