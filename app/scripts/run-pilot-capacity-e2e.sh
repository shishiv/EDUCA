...
sed -i \
  ...
  -e 's#site_url = "http://127.0.0.1:3000"#site_url = "https://educa-pilot-capacity.localhost"#' \
  -e 's#additional_redirect_urls = \["http://127.0.0.1:3000"\]#additional_redirect_urls = ["https://educa-pilot-capacity.localhost"]#' \
...
if (!namedUrl.hostname.endsWith('.localhost') || namedUrl.port !== '') throw new Error('PILOT_CAPACITY_PRE_RESET_NAMED_APP_URL_REQUIRED')
