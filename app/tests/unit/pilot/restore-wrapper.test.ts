...
function execute(status: string, environment: Partial<NodeJS.ProcessEnv> = {}) {
  ...
  return spawnSync('bash', [path.join(directory, 'app/scripts/run-pilot-restore-test.sh')], {
    encoding: 'utf8',
    env: { PATH: process.env.PATH, HOME: directory, FIXTURE_ROOT: directory, ...environment, NODE_ENV: 'test' },
  })
}
