...
class MemoryPilotInvitationAdapter implements PilotInvitationAdapter {
  readonly auditInputs: Array<{ invitation: UserLifecycleInvitation; role: UserLifecycleRole; schoolId: string | null }> = []
  ...
  constructor(
    ...
    private readonly auditSucceeds = true,
  ) {}
  ...
  async recordInvitationAudit(invitation: UserLifecycleInvitation, role: UserLifecycleRole, schoolId: string | null): Promise<boolean> {
    this.auditInputs.push({ invitation, role, schoolId })
