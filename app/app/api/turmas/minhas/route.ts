...
export interface TurmaWithDetails extends Pick<TurmaRow, 'id' | 'nome' | 'serie' | 'turno' | 'ano_letivo' | 'capacidade'> {
  escola: {
    id: string
    nome: string
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const turmasQuery = activeClassesQuery(supabase, limit, userProfile)
    ...
    return await classesAttendanceResponse(supabase, turmas ?? [])

  } catch (error) {
    logger.error('Error in minhas turmas API', error instanceof Error ? error : new Error('Erro desconhecido'))
...
type ClassesProfile = Pick<Database['public']['Tables']['users']['Row'], 'id' | 'tipo_usuario' | 'escola_id'>

function activeClassesQuery(supabase: SupabaseClient<Database>, limit: number, userProfile: ClassesProfile) {
...
type ClassDetailsRow = NonNullable<Awaited<ReturnType<typeof activeClassesQuery>>['data']>[number]

async function classesAttendanceResponse(supabase: SupabaseClient<Database>, turmas: ClassDetailsRow[]) {
  ...
  // Format response
  const turmasFormatted: TurmaWithDetails[] = turmas.map((turma) => ({
    id: turma.id,
    nome: turma.nome,
    serie: turma.serie,
    turno: turma.turno,
