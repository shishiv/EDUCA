...
export async function GET() {
  try {
    ...
    return await pendingAttendanceResponse(supabase, userProfile)

  } catch (error) {
    logger.error('Error in chamada pendentes API', error instanceof Error ? error : new Error('Erro desconhecido'))
...
type PendingAttendanceProfile = Pick<Database['public']['Tables']['users']['Row'], 'id' | 'tipo_usuario' | 'escola_id'>

async function pendingAttendanceResponse(
  supabase: SupabaseClient<Database>,
  userProfile: PendingAttendanceProfile
) {
