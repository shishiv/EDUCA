...
async function rollbackDemoImport(actor: PilotActor, batchId: string): Promise<NextResponse> {
...
function rollbackActorResponse(actor: PilotActor): NextResponse | null {
...
async function rollbackGovernedImport(
  actor: PilotActor,
  batchId: string,
  reason: string
): Promise<NextResponse> {
  assertSyntheticPilotSafety('import')
  const actorError = rollbackActorResponse(actor)
  ...
  const { data: rollbackRows, error: rollbackError } = await asPilotRpcClient(service).rpc(
    'pilot_rollback_synthetic_import_batch',
    { p_batch_id: batch.id, p_actor_user_id: actor.id, p_reason: reason },
  )
...
export async function POST(
  ...
  try {
    ...
    if (demoSandbox) {
      return await rollbackDemoImport(actor, batchId)
    }

    return await rollbackGovernedImport(actor, batchId, parsedBody.data.reason)
  } catch (error) {
