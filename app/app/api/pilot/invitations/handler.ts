...
export interface PilotInvitationAdapter {
  ...
  recordInvitationAudit(invitation: UserLifecycleInvitation, role: UserLifecycleRole, schoolId: string | null): Promise<boolean>
}
...
function createSupabasePilotInvitationAdapter(): PilotInvitationAdapter {
  ...
  return {
    ...
    async recordInvitationAudit(invitation, role, schoolId) {
      const supabase = await createClient()
      const { data, error } = await asPilotRpcClient(supabase).rpc<string>('write_pilot_audit_event', {
        p_event_type: 'user_invited',
        p_entity_type: 'user',
        p_entity_id: invitation.auth_user_id,
        p_escola_id: schoolId ?? undefined,
        p_metadata: { role },
      })
...
async function runPilotInvitation(adapter: PilotInvitationAdapter, input: InvitationRouteInput, actorId: string): Promise<NextResponse> {
  ...
  if (!await adapter.recordInvitationAudit(registration.invitation, input.role, input.schoolId)) {
