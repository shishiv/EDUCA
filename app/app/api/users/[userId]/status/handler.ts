...
function createUserStatusStore(): UserStatusStore {
  ...
  return {
    ...
    async audit(target) {
      const { data, error } = await asPilotRpcClient(await auditClient()).rpc('write_pilot_audit_event', { p_event_type: 'user_status_updated', p_entity_type: 'user', p_entity_id: target.id, p_escola_id: target.escola_id ?? undefined, p_metadata: {} })
