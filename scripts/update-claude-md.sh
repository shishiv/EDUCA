...
# Update CLAUDE.md with latest project information and Specify workflow integration
# Usage: ./scripts/update-claude-md.sh
...
main() {
    ...
    log_info "Updating CLAUDE.md with Specify workflow integration..."

    # Backup existing CLAUDE.md
    if [[ -f "$claude_md_file" ]]; then
        cp "$claude_md_file" "$claude_md_file.backup"
        log_success "Backed up existing CLAUDE.md to CLAUDE.md.backup"
    fi

    # Update CLAUDE.md with Specify workflow
    update_claude_md_content "$claude_md_file"

    log_success "CLAUDE.md updated with Specify workflow integration!"
    log_info "Key additions:"
    log_info "  - /specify, /plan, /tasks commands documented"
    log_info "  - Specify workflow process outlined"
    log_info "  - Script usage instructions"
    log_info "  - Constitutional development principles"
}

update_claude_md_content() {
    local claude_md_file="$1"

    # Get existing content up to MVP Implementation Status section
    local existing_content=""
    if [[ -f "$claude_md_file" ]]; then
        existing_content=$(awk '/^## MVP Implementation Status/{exit} {print}' "$claude_md_file")
    fi

    # Create updated CLAUDE.md with Specify integration
...
EOF
    ...
    log_success "CLAUDE.md has been updated with complete Specify framework integration"
}
